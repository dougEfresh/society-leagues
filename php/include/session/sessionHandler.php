...
class MySqlSessionHandler{
    ...
    public function open() {
        ...
        $limit = time() - (3600 * 24 * 25);
        $sql_map = sprintf("delete from session_map where session_id in (select id from %s where timestamp < '%s')",
            $this->dbTable, $limit
        );
        $this->dbConnection->query($sql_map);
    ...
    public function destroy($id) {
        $sql_map = sprintf("delete from session_map where session_id = '%s'", $this->dbConnection->escape_string($id));
        $this->dbConnection->query($sql_map);
