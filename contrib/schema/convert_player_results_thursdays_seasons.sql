...
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 3
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 5
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 9
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 13
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 15
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 18
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 21
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 23
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 27
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 31
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 33
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 49
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 58
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
...
join leagues.division d on d.division_type = 'EIGHT_BALL_THURSDAYS'
join leagues.player p on p.user_id=r.player_id and p.season_id = m.season_id and r.team_id=p.team_id and d.division_id = p.division_id
 where m.league_id = 2 and m.season_id = 65
) a
join
(
select r.result_id,r.match_id,r.player_id,r.team_id, games_won,r.match_number
